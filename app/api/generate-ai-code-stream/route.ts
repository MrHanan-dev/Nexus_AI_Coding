...
const modelMapping: { [key: string]: any } = {
  ...
  'google/gemini-2.0-flash': googleGenerativeAI,
  'google/gemini-2.0-pro': googleGenerativeAI,
  'google/gemini-2.5-pro': googleGenerativeAI,
...
const isGPT5 = (model: string) => model.includes('gpt-5');
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('[generate-ai-code-stream] API key status:', {
      groq: hasGroqKey,
    ...
    // Check if the requested model has an API key
    const modelProvider = model.split('/')[0];
    let hasRequiredKey = false;
    
    switch (modelProvider) {
      case 'groq':
        hasRequiredKey = hasGroqKey;
        break;
      case 'anthropic':
        hasRequiredKey = hasAnthropicKey;
        break;
      case 'google':
        hasRequiredKey = hasGeminiKey;
        break;
      case 'openai':
        hasRequiredKey = hasOpenAIKey;
        break;
      default:
        hasRequiredKey = hasGroqKey || hasAnthropicKey || hasGeminiKey || hasOpenAIKey;
    }
    
    if (!hasRequiredKey) {
      return new Response(
        JSON.stringify({
          error: `API key not configured for ${modelProvider}. Please add your ${modelProvider.toUpperCase()}_API_KEY to the .env file.`
        }),
        { 
          status: 401,
    ...
    (async () => {
      try {
        console.log('[generate-ai-code-stream] Starting background processing...');
        // Send immediate start status
        await sendProgress({ type: 'status', message: '🚀 Starting development...' });
...
5. Make the requested change immediately`;
        ...
        const actualModel = isAnthropic ? model.replace('anthropic/', '') : 
                           ...
                           (isGoogle ? 'gemini-2.0-flash' : model);
...
It's better to have 3 complete files than 10 incomplete files.`
        ...
        console.log('[generate-ai-code-stream] Making AI call with model:', actualModel);
        console.log('[generate-ai-code-stream] Provider:', modelProvider);
        ...
        let result;
        try {
          console.log('[generate-ai-code-stream] Attempting AI call with model:', actualModel);
          result = await Promise.race([
            streamText(streamOptions),
            timeoutPromise
          ]);
          console.log('[generate-ai-code-stream] AI call successful');
        } catch (aiError) {
          console.error('[generate-ai-code-stream] AI call failed:', aiError);
          
          // If Gemini fails, try with Groq as fallback
          if (isGoogle && hasGroqKey) {
            console.log('[generate-ai-code-stream] Falling back to Groq...');
            await sendProgress({ type: 'status', message: '🔄 Switching to backup AI model...' });
            
            const fallbackOptions = {
              ...streamOptions,
              model: groq('groq/llama3-8b-8192')
            };
            ...
            try {
              ...
              console.log('[generate-ai-code-stream] Fallback AI call successful');
            } catch (fallbackError) {
              console.error('[generate-ai-code-stream] Fallback AI call also failed:', fallbackError);
              throw fallbackError;
            }
          } else {
            throw aiError;
...
Provide the complete file content without any truncation. Include all necessary imports, complete all functions, and close all tags properly.`;
                ...
                const completionResult = await streamText({
                  ...
                  temperature: isGPT5(model) ? undefined : appConfig.ai.defaultTemperature
                });
